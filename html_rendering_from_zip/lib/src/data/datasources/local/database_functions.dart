...
class DataBaseFunctions {
    ...
    try {
      _db = await openDatabase(
        ...
        onCreate: (db, version) async {
          db.execute(
              'CREATE TABLE extracteddata (id INTEGER PRIMARY KEY, path TEXT)');
        },
      );
    } catch (e) {
  ...
  Future<Directory> getExtractedData() async {
    try {
      ...
      Archive archive =
          zipDecoder.decodeBytes(compressedData);
      ...
      if (e is ArchiveException) {
        log('is ArchiveException : $e');
      } else {
        log('is not ArchiveException : $e');
      }
      throw Exception();
    }
  }
  ...
  Future<void> insertExtractedData() async {
    ...
    final int count =
        await _db!.query('extracteddata').then((results) => results.length);
  ...
  Future<String?> getHtmlFilePath() async {
    ...
    List<Map<String, Object?>> list =
        await _db!.rawQuery('SELECT * FROM extracteddata');
